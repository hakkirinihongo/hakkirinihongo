...
とは限らない　è un costruzione neutra o leggermente formale: non è colloquiale strettissimo (come una slang), ma nemmeno particolarmente complicata da essere usata solo in contesti molto ufficiali. Nella forma cortese, si può rendere semplicemente con *～とは限りません* (negativo in -masu) senza cambiare sostanza. Spesso ricorre in spiegazioni, analisi o discussioni in cui si vuole puntualizzare che un’affermazione generale non vale sempre. 
...
- わけではない risponde a un’asserzione implicita del parlante od altrui. Per esempio, a chi dice che “Lui oggi non è venuto a scuola → quindi per forza deve stare male), servirà per negare quell'assunto.    Esempio:
